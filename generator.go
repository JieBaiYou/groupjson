...
// Generator 代码生成器，为结构体创建自定义序列化方法。
type Generator struct {
	// TypeName 目标结构体名称。
	TypeName string
	// PackageName 生成代码所属包名。
	PackageName string
	// SourceFile 源文件路径。
	SourceFile string
	// OutputFile 输出文件路径。
	...
	// TagName 用于标识分组的标签名。
	TagName string
	// StructComment 结构体原始注释，将保留在生成文件中。
	StructComment string
}

// NewGenerator 创建代码生成器实例，使用默认标签名"groups"。
...
// parseStructFields 解析结构体字段标签，提取分组和JSON信息。
...
// templateData 模板数据结构，包含代码生成所需信息。
type templateData struct {
	PackageName   string      // 包名
	TypeName      string      // 结构体名
	SourceFile    string      // 源文件
	Fields        []fieldInfo // 字段列表
	HasOmitEmpty  bool        // 是否使用omitempty
	HasOmitZero   bool        // 是否使用omitzero
	StructComment string      // 结构体注释
}

// generateCode 使用模板生成序列化代码。
